...
from resources.libs import extract, downloader, notify, debridit, traktit, allucit, loginit, net,diss, skinSwitch, uploadLog, yt, speedtest, wizard as wiz, addonwindow as pyxbmct
...
def apkMenu(name=None, url=None):
	...
	if not APKFILE == 'http://':
		if url == None:
			TEMPAPKFILE = wiz.textCache(diss.APKFILE)
			if TEMPAPKFILE == False: APKWORKING  = wiz.workingURL(diss.APKFILE)
		else:
...
def addonMenu(name=None, url=None):
	if not ADDONFILE == 'http://':
		if url == None:
			TEMPADDONFILE = wiz.textCache(diss.ADDONFILE)
			if TEMPADDONFILE == False: ADDONWORKING  = wiz.workingURL(diss.ADDONFILE)
		else:
...
def youtubeMenu(name=None, url=None):
	if not YOUTUBEFILE == 'http://':
		if url == None:
			TEMPYOUTUBEFILE = wiz.textCache(diss.YOUTUBEFILE)
			if TEMPYOUTUBEFILE == False: YOUTUBEWORKING  = wiz.workingURL(diss.YOUTUBEFILE)
		else:
...
def advancedWindow(url=None):
	if not ADVANCEDFILE == 'http://':
		if url == None:
			TEMPADVANCEDFILE = wiz.textCache(diss.ADVANCEDFILE)
			if TEMPADVANCEDFILE == False: ADVANCEDWORKING  = wiz.workingURL(diss.ADVANCEDFILE)
		else:
...
FOCUS_BUTTON_COLOR = diss.FOCUS_BUTTON_COLOR
DESCOLOR           = diss.DESCOLOR
DES_T_COLOR        = diss.DES_T_COLOR
MAIN_BUTTONS_TEXT  = diss.MAIN_BUTTONS_TEXT
OTHER_BUTTONS_TEXT = diss.OTHER_BUTTONS_TEXT
LIST_TEXT          = diss.LIST_TEXT
HIGHLIGHT_LIST     = diss.HIGHLIGHT_LIST

net      = net.Net()
window   = pyxbmct.AddonDialogWindow('')
EXIT     = os.path.join(ART , '%s.png' % diss.EXIT_BUTTON_COLOR)
...
NOTXT    = os.path.join(ART , '%s.gif'% diss.NO_TXT_FILE)
...
wiz_title =  pyxbmct.Label('[COLOR %s][B]%s[/B][/COLOR]' % (diss.WIZTITLE_COLOR ,diss.WIZTITLE))
window.placeControl(wiz_title, -5, 11, 7, 20)

wiz_ver =  pyxbmct.Label('[COLOR %s]Version: [COLOR %s][B]%s[/B][/COLOR]' % (diss.VERTITLE_COLOR,diss.VER_NUMBER_COLOR,VERSION))
...
if   mode=='index'          : index()
    ...
    from resources.lib import trye
